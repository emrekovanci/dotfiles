...
(defun my-c-mode-hook ()
  (keymap-local-set "M-o" #'ff-find-other-file)
  (c-set-style "k&r")
  ...
  (defun lazygit ()
    ...
    (let ((default-directory
           (if-let ((project (project-current)))
             ...
             default-directory)))
      (shell-command (concat "wt.exe -w 0 -d \"" (expand-file-name default-directory) "\" lazygit")))))
...
(custom-set-faces
 ...
 '(mode-line ((t (:background "white" :foreground "black" :box (:line-width (1 . 1) :color "black" :style flat-button)))))
 '(mode-line-inactive ((t (:background "grey90" :foreground "grey50" :box (:line-width (1 . 1) :color "grey75") :weight light))))
